...
- **测试:** `package.json` 中目前未配置自动化测试框架 (Vitest/Jest)。
  - _行动:_ 如果被要求编写测试，请先验证是否需要安装运行器。否则，通过本地运行应用来验证更改。
...
- **注释:** 现有注释通常为 **中文** (例如 `// 获取全部电影的观影记录`)。修改现有注释块时请保持此风格。
...
7.  **环境:** 注意 `.env` 文件中的配置，如 IGDB、TMDB 密钥，开发时确保本地环境配置正确。
...
- **API 路由 (`server/api/`)**:
  ...
  - 使用 `server/utils/` 中的辅助函数（如 `igdb.js`）来封装复杂的外部 API 调用。
- **页面组件 (`pages/`)**:
